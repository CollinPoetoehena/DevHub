...
- **VERY IMPORTANT: Stay the expert:** use AI as support, understand and verify what it generates, keep reasoning through complex problems yourself, and remember that strategic choices, system design, and interpretation remain human work — AI can and will make mistakes, and you're the one who has to catch them. AI is a force multiplier, NOT a replacement for human engineering.
  ...
  - **Coding is only one piece of building good software:** generating code faster, on its own, doesn't produce reliable, production-grade systems. Robust software requires broader engineering thinking that goes well beyond the code itself — designing for **scalability** as usage/data grows, proactively **identifying bottlenecks** (performance, architecture, process) before they become real problems, **anticipating future features** so today's design doesn't box you in later, and carefully weighing whether a change carries **risk of impacting customers** (downtime, data loss, breaking changes, regressions), etc., before shipping it. That kind of judgment comes from engineering fundamentals — system design, testing/rollout strategy, monitoring, and experience, etc. — not from how fast you (or an AI) can generate code. AI can support this thinking (e.g. brainstorming scaling approaches, surfacing edge cases, reviewing a rollout plan), but it can't replace the judgment needed to make the right call — that responsibility, as with everything else on this page, stays with you (i.e. *you stay the expert*).
