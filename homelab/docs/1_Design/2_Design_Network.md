...
- **nftables** — handles NAT (masquerading) and firewall rules. It is built into the Linux kernel's netfilter framework — no extra software needed.
...
**Why nftables and not iptables or another alternative like firewalld or ufw?** nftables is the modern replacement for iptables, offering a more consistent and flexible syntax, better performance, and easier maintenance. iptables is still widely used and supported, but it is considered legacy, and its syntax can be cumbersome for complex rulesets. firewalld is a higher-level abstraction over iptables/nftables, providing dynamic rule management and a simpler interface, but it adds an extra layer of complexity and is less transparent for learning purposes. ufw is an even higher-level abstraction aimed at simplifying firewall management for end users, but it hides the underlying rules and is not suitable for learning or fine-grained control. For a homelab router where the goal is to understand and control the underlying firewall rules, nftables is the most suitable choice. Also, the configuration and maintenance of nftables is straightforward and fully transparent, making it ideal for educational purposes in a homelab environment. Finally, nftables is the modern, future-proof choice for Linux firewall management. See for all details the [router Ansible role](../../ansible/roles/router/README.md) (specifically the [`firewall.yml` task](../../ansible/roles/router/tasks/firewall.yml).
